...
class RCPTrack(Track):
    ...
    def initTrack(self, description, gridsize, scale, savepath=None):
        ...
        for i in range(len(description)):
            ...
            assert (current_index[0] < gridsize[1]) & (current_index[1]
                                                       < gridsize[0])
    ...
    def drawTrack(self, img=None, show=False):
        ...
        straight = cv2.rectangle(straight, (0, 0),
                                 (gs - 1, int(deadzone * gs)), color_side, -1)
        straight = cv2.rectangle(straight, (0, int((1 - deadzone) * gs)),
                                 (gs - 1, gs - 1), color_side, -1)
        WE = straight

        # prepare turn section (SE)
        turn = 255 * np.ones([gs, gs, 3], dtype='uint8')
        turn = cv2.rectangle(turn, (0, 0), (int(deadzone * gs), gs - 1),
                             color_side, -1)
        turn = cv2.rectangle(turn, (0, 0), (gs - 1, int(deadzone * gs)),
                             color_side, -1)
        turn = cv2.rectangle(turn, (0, 0), (int(0.5 * gs), int(0.5 * gs)),
                             color_side, -1)
        turn = cv2.circle(turn, (int(0.5 * gs), int(0.5 * gs)),
                          int((0.5 - deadzone) * gs), (255, 255, 255), -1)
        turn = cv2.circle(turn, (gs - 1, gs - 1), int(deadzone * gs),
                          color_side, -1)
        SE = turn
        ...
        for i in range(cols):
            for j in range(rows):
                ...
                if (signature == 'WE'):
                    ...
                    M = cv2.getRotationMatrix2D((gs / 2, gs / 2),
                                                lookup_table[signature], 1.01)
    ...
    def initRaceline(self, start, start_direction, offset=None, filename=None):
        ...
        for i in range(len(self.grid_sequence)):
            if start[0] == self.grid_sequence[i][0] and start[
                    1] == self.grid_sequence[i][1]:
                start_seq = i
        ...
        self.start_pos = ((0.5 + start[0]) * self.scale,
                          (0.5 + start[1]) * self.scale)
        ...
        while (1):
            ...
            exit_ctrl_pt += offset[index] * np.array(lookup_table_right[exit],
                                                     dtype='float') / 2
            index += 1
        ...
        tck, u = splprep(pts.T,
                         u=np.linspace(0, self.track_length_grid,
                                       self.track_length_grid + 1),
                         s=smoothing_factor,
                         per=1)
        #NOTE
    ...
    def generateSpeedProfile(self,
        ...
        curvature = 1.0 / (
            _norm(dr)**3 /
            (_norm(dr)**2 * _norm(ddr)**2 - np.sum(dr * ddr, axis=0)**2)**0.5)
        ...
        for i in range(min_xx, min_xx + n_steps):
            ...
            if ((mu * g)**2 - a_lat**2) > 0:
                ...
                a_lon = min(acc_max_fun(v2[i % n_steps]),
                            a_lon_available_traction)
        ...
        for i in np.linspace(min_xx, min_xx - n_steps, n_steps + 2):
            ...
            (x_i, y_i) = splev(xx[i % n_steps], self.raceline, der=0)
            (x_i_1, y_i_1) = splev(xx[(i - 1 + n_steps) % n_steps],
                                   self.raceline,
                                   der=0)
            # distance between two steps
            ds = dist((x_i, y_i), (x_i_1, y_i_1))
            #print(ds)
            v3[(i - 1 + n_steps) % n_steps] = min(
                (v3[i % n_steps]**2 + 2 * a_lon * ds)**0.5,
        ...
        return {
            'speed_profile_fun': speed_profile_fun,
            'min_v': min_v,
            'max_v': max_v
        }
    ...
    def getOrcaStyleTrack(self):
        ...
        s = self.s_vec = s_vec = np.linspace(0, self.raceline_len_m,
        ...
        return (N, X, Y, s, phi, kappa, diff_s, d_upper, d_lower,
                border_angle_upper, border_angle_lower)
    ...
    def preciseTrackBoundary(self, coord=None, heading=None, s=None, n=None):
        if (not coord is None):
            ...
            dr = self.r[(index + 1) %
                        self.discretized_raceline_len] - self.r[index]
            ...
            right = splev(s,
                          self.raceline_right_boundary_fun)[0].item() + offset
            return (left, right)
        elif (not s is None):
    ...
    def preciseTrackBoundaryGrid(self, coord, heading):
        ...
        if grid_type in straights:
            ...
            while (flag_in_limit):
                ...
                flag_in_limit = RCPTrack.checkTrackBoundary(self,
                                                            left_point) > 0
                left += step_size
    ...
    def drawRacelineWithColor(self,
                              lineColor=(0, 0, 255),
                              img=None,
                              thickness=3,
                              s_to_color=lambda s: 0):
        '''
        ...
        # render different color based on speed
        # slow - red, fast - green (BGR)
        getColor = lambda s: (0, int(s_to_color(s) * 255),
    ...
    def localTrajectory(self, state, wheelbase=90e-3, return_u=False):
        ...
        if self.last_u is None:
            ...
            for i in range(len(self.grid_sequence)):
                if nondim[0] == self.grid_sequence[i][0] and nondim[
                        1] == self.grid_sequence[i][1]:
                    seq = i
                    break
        ...
        raceline_point = splev(min_fun_x % self.track_length_grid,
                               self.raceline)
        ...
        vec_curvature = splev(min_fun_x % self.track_length_grid,
                              self.raceline,
                              der=2)
        ...
        request_velocity = self.targetVfromU(min_fun_x %
                                             self.track_length_grid)
        ...
        if return_u:
            return (raceline_point,
                    copysign(abs(min_fun_val)**0.5,
                             cross_theta), atan2(der[1], der[0]),
                    copysign(norm_curvature, cross_curvature),
                    request_velocity, min_fun_x % self.track_length_grid)
        else:
            return (raceline_point,
                    copysign(abs(min_fun_val)**0.5,
                             cross_theta), atan2(der[1], der[0]),
                    copysign(norm_curvature,
                             cross_curvature), request_velocity)
    ...
    def reconstructRaceline(self):
        ...
        # construct boundary
        self.raceline_left_boundary_fun, self.raceline_right_boundary_fun = self.createBoundary(
            self.raceline_s, self.raceline_len_m)
        return
    ...
    def createBoundary(self, raceline_s, raceline_len_m):
        ...
        phi = np.arctan2(dr[1, :], dr[0, :])
        boundary = np.array([
            self.preciseTrackBoundaryGrid(r, h) for (r, h) in zip(rr.T, phi.T)
        ])

        boundary[-1] = boundary[0]
        left_boundary_fun, _ = splprep(boundary[:, 0].reshape(1, -1),
                                       u=ss,
                                       s=0,
                                       per=1)
        right_boundary_fun, _ = splprep(boundary[:, 1].reshape(1, -1),
                                        u=ss,
                                        s=0,
                                        per=1)
    ...
    def getRefPoint(self, state, p, dt, reverse=False):
        ...
        # curvature needs to be signed to indicate whether signage target angular velocity
        # a cross product gives right signage for omega, this is indep of track direction since it's calculated based off vehicle orientation
        cross_curvature = der[0, :] * vec_curvature[1, :] - der[
            1, :] * vec_curvature[0, :]
        ...
        return offset, e_heading, np.array(v_vec), np.array(
            k_signed_vec), np.array(coord_vec), True
