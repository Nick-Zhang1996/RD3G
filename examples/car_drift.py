...
class CarDrift(ResidualGame):
    ...
    def __init__(self, config: ResidualGameConfig):
        ...
        self.x0 = np.array(
            [[0, 0, radians(17), vx, vy, r, theta, Br],
             [self.ds_ref, 0,
              radians(17), vx, vy, r, theta, Br]])
        self.track = Skidpad()
        ...
        self.car_img_vec = [
            mpimg.imread(img_path_orange),
            mpimg.imread(img_path_blue)
        ]
    ...
    def setup(self):
        ...
        self.x0 = np.array(
            [[0, 0, self.mu_ref, vx, vy, r, theta, Br],
             [self.ds_ref, 0, self.mu_ref, vx, vy, r, theta, Br]])
        return
    ...
    def _animation(self, U, X=None, gif_prefix=''):
        ...
        for states in X:
            car_pose_vec.append([
                self.getCartesianFromFrenet(states[i]) for i in range(self.N)
            ])
        im_vec = []
        for i in range(self.N):
            rotated_car_img = np.clip(
                rotate(self.car_img_vec[i],
    ...
    def findSaddlePoint(self):
        ...
        def ctrl_to_dvxdvy(x, *data):
            ...
            #dr = 1/Iz*(Ffy*np.cos(theta)*lf - Fry * lr)
            print(
                f'\t theta {degrees(theta):.2f}deg, Br {degrees(theta):.2f}deg, dvx {dvx:.2f}, dvy {dvy:.2f}'
            )
            return [dvx, dvy]
        ...
        def vy_to_dr(x, *data):
            ...
            r = k_s * ds
            print(
                f'solver step: vx {vx:.2f}, vy {vy:.2f}, r {np.pi*2/r:.2f}sec/rev'
            )
            ...
            theta, Br = fsolve(ctrl_to_dvxdvy, (-radians(10), radians(20)),
                               args=(vx, vy, r, n, k_s))
            Fry = Tmax * np.sin(Br)
            Frx = Tmax * np.cos(Br)
            Ffy = Bf = -np.arctan((vy + lf * r) / vx) + theta
        ...
        ds = (vx**2 + vy**2)**0.5 / (1 - n * k_s)
        r = k_s * ds
        theta, Br = fsolve(ctrl_to_dvxdvy, (-radians(10), radians(10)),
                           args=(vx, vy, r, n, k_s))
        Fry = Tmax * np.sin(Br)
        Frx = Tmax * np.cos(Br)
        Ffy = Bf = -np.arctan((vy + lf * r) / vx) + theta
        ...
        if (vx > 0 and vy < 0 and theta < 0 and Br > 0 and Br < np.pi / 2
                and np.linalg.norm(dvx) < 1e-3 and np.linalg.norm(dvy) < 1e-3
                and np.linalg.norm(dr)):
            print(
                f' new saddle point: vx = {vx:.2f}, vy = {vy:.2f}, theta = {degrees(theta):.2f}deg, Br = {degrees(Br):.2f}deg'
            )
        else:
            print(
                f' bad saddle point: vx = {vx:.2f}, vy = {vy:.2f}, theta = {degrees(theta):.2f}deg, Br = {degrees(Br):.2f}deg'
            )
    ...
    def buildObjectiveJacobian(self):
        ...
        J_eq = (mu_i - self.mu_ref)**2 + self.vx_cost * (
            vx_i -
            self.vx_ref)**2 + self.n_cost * n_i**2 + self.control_cost * (
                dsteer_i**2 + dB_i**2) + self.ds_cost * (
                    ds_sign * (s_i - s_j) -
                    self.ds_ref)**2 + self.dmu_cost * (mu_i - mu_j)**2
        dyn.symDerJ(J_eq)
