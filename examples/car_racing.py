...
class CarRacing(ResidualGame):
    ...
    def _visualize(self, u, x=None):
        ...
        ax.imshow(self.img_track,
                  extent=[
                      self.track.x_min, self.track.x_max, self.track.y_min,
                      self.track.y_max
                  ])
        for i in range(self.N):
    ...
    def _animation(self, U, X=None, gif_prefix=''):
        ...
        if (self.sprite_visualization):
            ...
            for states in X:
                car_pose_vec.append(
                    [self.curv2Cart(states[i]) for i in range(self.N)])
            ...
            for i, color in zip(range(self.N), color_vec):
                ...
                # width, height
                box_vec.append(
                    plt.Rectangle(pos_vec[0] - np.array([0.2 / 2, 0.1 / 2]),
                                  0.2,
                                  0.1,
                                  angle=angle_vec[0],
                                  color=color,
    ...
    def df_dx(self, x, u, i):
        ...
        dfdx = np.array([[
            0,
            cos(x3) / (-k_s * x2 + 1), k_s * x1 * cos(x3) / (-k_s * x2 + 1)**2,
            -x1 * sin(x3) / (-k_s * x2 + 1)
        ], [0, 0, 0, 0], [0, sin(x3), 0, x1 * cos(x3)],
                         [
                             0, -k_s * cos(x3) / (-k_s * x2 + 1) - u0 / x1**2,
                             -k_s**2 * x1 * cos(x3) / (-k_s * x2 + 1)**2,
    ...
    def cart2Curv(self, cart, guess_s=None):
        ...
        def dist(s):
            val = np.linalg.norm(
                np.array(
                    splev(s % self.track.raceline_len_m,
                          self.track.raceline_s,
                          der=0)).flatten() - np.array([x, y]))
            return val
        ...
        r = np.array(
            splev(s % self.track.raceline_len_m, self.track.raceline_s, der=0))
        dr = np.array(
            splev(s % self.track.raceline_len_m, self.track.raceline_s, der=1))
