...
class SymbolicDynamics:
    ...
    def calcDerL(self, x0, u0, subs_dict={}):
        ...
        dldxdx_val = np.array([[eq.evalf(subs=subs_dict) for eq in row]
                               for row in self.lxx])
        dldudu_val = np.array([[eq.evalf(subs=subs_dict) for eq in row]
                               for row in self.luu])
        dldudx_val = np.array([[eq.evalf(subs=subs_dict) for eq in row]
                               for row in self.lux])
...
class MultiAgentSymbolicDynamics:
    ...
    def __init__(self, n, m, N):
        ...
        self.n = n
        self.m = m
        self.x = [[symbols(f'x{i}_{agent}') for i in range(n)]
                  for agent in range(N)]
        self.u = [[symbols(f'u{i}_{agent}') for i in range(m)]
                  for agent in range(N)]
