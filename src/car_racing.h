...
public:
  CarRacing(const int _N, const int _T, const Scalar _dt, const Scalar _rho,
            ...
            const Matrix _h_Qh, const Matrix _target_y,
            const Scalar _collision_radius, const int _max_iter,
            const bool _verbose)
      : ResidualGame(_N, _T, _dt, _rho, _rho_b, _bc_a, _bc_b, _tolerance,
                     _backtracking_max_iter, _max_iter, _verbose),
        J_Qr(_J_Qr), J_Q(_J_Q), J_R(_J_R), h_Qh(_h_Qh), target_y(_target_y),
        car_size(_collision_radius) {
    /*
    cout << "N = " << N;
    cout << "T = " << T;
    cout << "n = " << n;
  ...
  void set_curvature_vector(
      const std::vector<std::tuple<Scalar, Scalar>> in_curvature_vec) {
    curvature_vec = std::move(in_curvature_vec);
  }

  Scalar curvature_fun(Scalar s) const {
    s = fmod(s, std::get<0>(curvature_vec.back()));
    auto it =
        std::lower_bound(curvature_vec.begin(), curvature_vec.end(), s,
  ...
  Matrix f(const Matrix x, const Matrix u, const int i) const {
    ...
    Matrix dx = (Matrix(n, 1) << x(1, 0) * cos(x(3, 0)) / (1 - x(2, 0) * k_s),
                 u(1, 0), x(1, 0) * sin(x(3, 0)),
                 u(0, 0) / x(1, 0) -
                     k_s * (x(1, 0) * cos(x(3, 0)) / (1 - x(2, 0) * k_s)))
                    .finished();
  ...
  Matrix df_dx(const Matrix x, const Matrix u, const int i) const {
    ...
    // dfdx
    Matrix A =
        (Matrix(n, n) << 0, cos(x(3, 0)) / (-k_s * x(2, 0) + 1),
         k_s * x(1, 0) * cos(x(3, 0)) / sqr(-k_s * x(2, 0) + 1),
         -x(1, 0) * sin(x(3, 0)) / (-k_s * x(2, 0) + 1), 0, 0, 0, 0, 0,
         sin(x(3, 0)), 0, x(1, 0) * cos(x(3, 0)), 0,
         -k_s * cos(x(3, 0)) / (-k_s * x(2, 0) + 1) - u(0, 0) / sqr(x(1, 0)),
         -k_s * k_s * x(1, 0) * cos(x(3, 0)) / sqr(-k_s * x(2, 0) + 1),
         k_s * x(1, 0) * sin(x(3, 0)) / (-k_s * x(2, 0) + 1))
            .finished();
  ...
  Matrix df_du(const Matrix x, const Matrix u, const int i) const {
    ...
    Matrix B = (Matrix(n, m) << 0, 0, 0, 1, 0, 0, 1 / x(1, 0), 0).finished();
    if (B.hasNaN()) {
  ...
  Scalar h(const Matrix x_i, const Matrix x_j) const {
    return -sqr(x_i(0, 0) - x_j(0, 0)) - sqr(x_i(2, 0) - x_j(2, 0)) +
           car_size * car_size;
  }
  ...
  Matrix J_x_ref_fun(int i) const {
    Matrix mtx(n, 1);
  ...
  Matrix J(const Matrix x_k, const Matrix u_k_i, int i) const {
    // int j = 1-i;
    return (x_k.row(i).transpose() - J_x_ref_fun(i)).transpose() * J_Qr *
               (x_k.row(i).transpose() - J_x_ref_fun(i)) +
           x_k.row(i) * J_Q * x_k.row(i).transpose() +
           u_k_i.transpose() * J_R * u_k_i;
  }
  Matrix dJi_dxi(const Matrix x_k, const Matrix u, int i) const {
    return 2 * (x_k.row(i).transpose() - J_x_ref_fun(i)).transpose() * J_Qr +
           2 * x_k.row(i).transpose().transpose() * J_Q;
  }
