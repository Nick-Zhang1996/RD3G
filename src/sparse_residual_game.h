...
public:
  ...
  Matrix dL_du(const Matrix x_k, const Matrix u_k_i, const Matrix x_k1_i,
               const Matrix h_k_plus_mask, const Matrix lamda_k,
               const Matrix mu_k, const int i) const {
    return dJi_du(x_k, u_k_i, i) +
           lamda_k.row(i) * df_du(x_k.row(i).transpose(), u_k_i, i);
  }
  ...
  Matrix dLLi_dxi(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                  const std::vector<Matrix> &h_plus_mask,
                  const std::vector<Matrix> &lamda,
                  const std::vector<Matrix> &mu, const int i) const {
    // TODO is this the best approach?
    Matrix der(1, T * n);
    der.setZero();
    // dLLi_dxi
    for (int k = 1; k < T; k++) {
      der.template block<1, n>(0, (k - 1) * n) =
          dL_dx_ik(x[k - 1], u[k].row(i).transpose(), x[k].row(i).transpose(),
                   h_plus_mask[k - 1], lamda[k], mu[k - 1], i) -
          lamda[k - 1].row(i);
    }
    // dLLi_dxi_T
    der.template block<1, n>(0, (T - 1) * n) =
        -lamda[T - 1].row(i) + dJi_dxi(x[T - 1], Matrix::Zero(m, 1), i);
    for (int j = 0; j < N; j++) {
      if (i == j) {
        continue;
      }
      if (h_plus_mask[T - 1](i, j)) {
        der.template block<1, n>(0, (T - 1) * n) +=
            mu[T - 1](i, j) *
            dh_dxi(x[T - 1].row(i).transpose(), x[T - 1].row(j).transpose());
      } else {
        der.template block<1, n>(0, (T - 1) * n) +=
            -1 / rho *
            min(1.0 /
                    h(x[T - 1].row(i).transpose(), x[T - 1].row(j).transpose()),
                1e10) *
            dh_dxi(x[T - 1].row(i).transpose(), x[T - 1].row(j).transpose());
      }
    }
  ...
  // mu_k_i_j: 1..T T*N*N NOTE starts from 1
  // TODO obsolete
  Matrix dLLi_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                 const std::vector<Matrix> &h_plus_mask,
                 const std::vector<Matrix> &lamda,
                 const std::vector<Matrix> &mu, const int i) const {
    ...
    // dLLi_dxi
    for (int k = 1; k < T; k++) {
      der.template block<1, n>(0, (k - 1) * N * n + i * n) =
          dL_dx_ik(x[k - 1], u[k, i], x[k].row(i).transpose(),
                   h_plus_mask[k - 1], lamda[k], mu[k - 1], i) -
          lamda[k - 1].row(i);
    }
    // dLLi_dxi_T
    der.template block<1, n>(0, (T - 1) * N * n + i * n) =
        -lamda[T - 1].row(i) + dJi_dxi(x[T - 1], Matrix::Zero(m, 1), i);
    for (int j = 0; j < N; j++) {
      if (i == j) {
        continue;
    ...
    for (int j = 0; j < N; j++) {
      ...
      for (int k = 1; k < T; k++) {
        if (h_plus_mask[k - 1](i, j)) {
          der.template block<1, n>(0, (k - 1) * N * n + j * n) =
              mu[k - 1](i, j) *
              dh_dxj(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());
        } else {
          der.template block<1, n>(0, (k - 1) * N * n + j * n) =
              -1.0 / rho *
              min(1.0 / h(x[k - 1].row(i).transpose(),
                          x[k - 1].row(j).transpose()),
                  1e10) *
              dh_dxj(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());
        }
        der.template block<1, n>(0, (k - 1) * N * n + j * n) +=
            dJi_dxj(x[k - 1], u[k].row(i).transpose(), i, j);
      }
      const int k = T;
      if (h_plus_mask[k - 1](i, j)) {
        der.template block<1, n>(0, (k - 1) * N * n + j * n) =
      ...
      der.template block<1, n>(0, (k - 1) * N * n + j * n) +=
          dJi_dxj(x[k - 1], Matrix::Zero(m, 1), i, j);
    }
    return der;
  }

  Matrix dLLi_dui(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                  const std::vector<Matrix> &h_plus_mask,
                  const std::vector<Matrix> &lamda,
                  const std::vector<Matrix> &mu, const int i) const {
    ...
    // dLLi_dui_0
    der.template block<1, m>(0, 0) =
        dJi_du(x0, u[0].row(i).transpose(), i) +
        lamda[0].row(i) *
            df_du(x0.row(i).transpose(), u[0].row(i).transpose(), i);

    // dLLi_dui_k
    for (int k = 1; k < T; ++k) {
      der.template block<1, m>(0, k * m) =
          dJi_du(x[k - 1], u[k].row(i).transpose(), i) +
          lamda[k].row(i) *
              df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i);
    }
  ...
  // TODO obsolete
  Matrix dLLi_du(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                 const std::vector<Matrix> &h_plus_mask,
                 const std::vector<Matrix> &lamda,
                 const std::vector<Matrix> &mu, const int i) const {
    throw std::runtime_error("obsolete function called");
    ...
    // dLLi_dui_k
    for (int k = 1; k < T; ++k) {
      der.template block<1, m>(0, i * m + k * N * m) =
          dJi_du(x[k - 1], u[k].row(i).transpose(), i) +
          lamda[k].row(i) *
              df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i);
    }
  ...
  Matrix dBh_dxi(const Matrix &x_i, const Matrix &x_j) const {
    // Calculate dBh/dxi
    return -1.0 / (rho * h(x_i, x_j)) * dh_dxi(x_i, x_j);
  }

  Matrix dBh_dxj(const Matrix &x_i, const Matrix &x_j) const {
    // Calculate dBh/dxj
    return -1.0 / (rho * h(x_i, x_j)) * dh_dxj(x_i, x_j);
  }
  Matrix dBh_dxi_dxi(const Matrix &x_i, const Matrix &x_j) const {
    ...
    Matrix val = 1.0 / (rho * h_val) *
                 (-dhdxi_dxi + 1.0 / h_val * dhdxi.transpose() * dhdxi);
    return val;
  }
  Matrix dBh_dxi_dxj(const Matrix &x_i, const Matrix &x_j) const {
    // Calculate h, dh/dxi, and dhdxi_dxi
    Scalar h_val = h(x_i, x_j);
    Matrix dhdxi = dh_dxi(x_i, x_j);
    ...
    // Calculate dBh/dxi_dxi
    Matrix val = 1.0 / (rho * h_val) *
                 (-dhdxi_dxj + 1.0 / h_val * dhdxi.transpose() * dhdxj);
    return val;
  }
  Matrix dBh_dxj_dxj(const Matrix &x_i, const Matrix &x_j) const {
    // Calculate h, dh/dxi, and dhdxi_dxi
    Scalar h_val = h(x_i, x_j);
    Matrix dhdxj = dh_dxj(x_i, x_j);
    ...
    // Calculate dBh/dxi_dxi
    Matrix val = 1.0 / (rho * h_val) *
                 (-dhdxj_dxj + 1.0 / h_val * dhdxj.transpose() * dhdxj);
    return val;
  }
  Matrix dF_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
    ...
    dFdx.template block<n, n>(0, start_idx_1) =
        df_dx(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i);
    dFdx.template block<n, n>(0, start_idx_2) = -Matrix::Identity(n, n);

    return dFdx;
  }

  Matrix dF0_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                const int i) const {
    // Initialize dF_dx matrix
    Matrix dFdx = Matrix::Zero(n, T * N * n);
  ...
  Matrix dh_dx(const std::vector<Matrix> &x, const int k, const int i,
               const int j) const {
    ...
    dhdx.template block<1, n>(0, start_idx_1) =
        dh_dxi(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());
    dhdx.template block<1, n>(0, start_idx_2) =
        dh_dxj(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());

    return dhdx;
  }

  Matrix dLLi_dxi_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                     const std::vector<Matrix> &h_plus_mask,
                     const std::vector<Matrix> &lamda,
                     const std::vector<Matrix> &mu, const int i) const {
    // cout << "dLLi_dxdx " << endl;
    ...
    auto submtx = [&](int k, int j) {
      return dLL_dxi_dx.template block<n, n>((k - 1) * n,
                                             (k - 1) * N * n + j * n);
    };
    ...
    for (int k = 1; k < T; ++k) {
      ...
      for (int j = 0; j < N; ++j) {
        ...
        if (h_plus_mask[k - 1](i, j)) {
          mtx += mu[k - 1](i, j) * dh_dxi_dxi(x[k - 1].row(i).transpose(),
                                              x[k - 1].row(j).transpose());
        } else {
          mtx += dBh_dxi_dxi(x[k - 1].row(i).transpose(),
                             x[k - 1].row(j).transpose());
        }
      }
    }
    ...
    for (int j = 0; j < N; ++j) {
      ...
      if (h_plus_mask[T - 1](i, j)) {
        mtx += mu[T - 1](i, j) * dh_dxi_dxi(x[T - 1].row(i).transpose(),
                                            x[T - 1].row(j).transpose());
      } else {
        mtx += dBh_dxi_dxi(x[T - 1].row(i).transpose(),
                           x[T - 1].row(j).transpose());
      }
    }
    ...
    for (int k = 1; k < T; ++k) {
      for (int j = 0; j < N; ++j) {
        ...
        if (h_plus_mask[k - 1](i, j)) {
          val = dJi_dxi_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
                mu[k - 1](i, j) * dh_dxi_dxj(x[k - 1].row(i).transpose(),
                                             x[k - 1].row(j).transpose());
        } else {
          val = dJi_dxi_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
                dBh_dxi_dxj(x[k - 1].row(i).transpose(),
                            x[k - 1].row(j).transpose());
        }
        submtx(k, j) = val;
      }
    ...
    for (int j = 0; j < N; ++j) {
      ...
      Matrix val = Matrix::Zero(n, n);
      if (h_plus_mask[k - 1](i, j)) {
        val = dJi_dxi_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
              mu[k - 1](i, j) * dh_dxi_dxj(x[k - 1].row(i).transpose(),
                                           x[k - 1].row(j).transpose());
      } else {
        val = dJi_dxi_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
  ...
  // TODO obsolete
  Matrix dLLi_dxdx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                   const std::vector<Matrix> &h_plus_mask,
                   const std::vector<Matrix> &lamda,
                   const std::vector<Matrix> &mu, const int i) const {
    throw std::runtime_error("obsolete function called");
    ...
    auto submtx = [&](int k, int i, int j) {
      return dLL_dxdx.template block<n, n>((k - 1) * N * n + i * n,
                                           (k - 1) * N * n + j * n);
    };
        ...
        if (h_plus_mask[k - 1](i, j)) {
          val = dJi_dxi_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
                mu[k - 1](i, j) * dh_dxi_dxj(x[k - 1].row(i).transpose(),
                                             x[k - 1].row(j).transpose());
          submtx(k, j, j) =
              dJi_dxj_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
              mu[k - 1](i, j) * dh_dxj_dxj(x[k - 1].row(i).transpose(),
                                           x[k - 1].row(j).transpose());
        } else {
          val = dJi_dxi_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
                dBh_dxi_dxj(x[k - 1].row(i).transpose(),
          ...
          submtx(k, j, j) =
              dJi_dxj_dxj(x[k - 1], u[k].row(i).transpose(), i, j) +
              dBh_dxj_dxj(x[k - 1].row(i).transpose(),
                          x[k - 1].row(j).transpose());
        }
        submtx(k, i, j) = val;
      ...
      if (h_plus_mask[k - 1](i, j)) {
        val = dJi_dxi_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
              mu[k - 1](i, j) * dh_dxi_dxj(x[k - 1].row(i).transpose(),
                                           x[k - 1].row(j).transpose());
        submtx(k, j, j) =
            dJi_dxj_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
            mu[k - 1](i, j) * dh_dxj_dxj(x[k - 1].row(i).transpose(),
                                         x[k - 1].row(j).transpose());
      } else {
        val = dJi_dxi_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
              dBh_dxi_dxj(x[k - 1].row(i).transpose(),
                          x[k - 1].row(j).transpose());
        submtx(k, j, j) = dJi_dxj_dxj(x[k - 1], Matrix::Zero(m, 1), i, j) +
                          dBh_dxj_dxj(x[k - 1].row(i).transpose(),
                                      x[k - 1].row(j).transpose());
      }
      submtx(k, i, j) = val;
  ...
  Matrix dLLi_dxi_dmu(const std::vector<Matrix> &x,
                      const std::vector<Matrix> &u,
                      const std::vector<Matrix> &h_plus_mask,
                      const std::vector<Matrix> &lamda,
                      const std::vector<Matrix> &mu, int i) const {
    ...
    for (int k = 1; k < T + 1; ++k) {
      for (int j = 0; j < N; j++) {
        if (h_plus_mask[k - 1](i, j)) {
          Matrix dLLi_dxki_dmuijk =
              dh_dxi(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());
          dLL_dxi_dmu.template block<n, 1>((k - 1) * n,
                                           (k - 1) * N * N + i * N + j) =
              dLLi_dxki_dmuijk.transpose();
        }
      }
    }
  ...
  // TODO obsolete
  Matrix dLLi_dx_dmu(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                     const std::vector<Matrix> &h_plus_mask,
                     const std::vector<Matrix> &lamda,
                     const std::vector<Matrix> &mu, int i) const {
    throw std::runtime_error("obsolete function called");
    ...
    for (int k = 1; k < T + 1; ++k) {
      for (int j = 0; j < N; j++) {
        if (h_plus_mask[k - 1](i, j)) {
          Matrix dLLi_dxki_dmuijk =
              dh_dxi(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());
          Matrix dLLi_dxkj_dmuijk =
              dh_dxj(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose());

          dLL_dx_dmu.template block<n, 1>((k - 1) * N * n + i * n,
                                          (k - 1) * N * N + i * N + j) =
              dLLi_dxki_dmuijk.transpose();
          dLL_dx_dmu.template block<n, 1>((k - 1) * N * n + j * n,
                                          (k - 1) * N * N + i * N + j) =
              dLLi_dxkj_dmuijk.transpose();
        }
      }
    }
  ...
  // fill-in style
  template <typename Derived>
  void dr_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
             const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
             const std::vector<Matrix> &h_plus_mask,
             const MatrixBase<Derived> &mtx) const {
    int dim_x = N * T * n;
    int dim_u = N * T * m;
  ...
  // copy style
  Matrix dr_dx(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
               const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
  ...
  void dr_dx_fill_block(const std::vector<Matrix> &x,
                        const std::vector<Matrix> &u,
                        const std::vector<Matrix> &lamda,
                        const std::vector<Matrix> &mu,
                        const std::vector<Matrix> &h_plus_mask, SpMatrix &mtx,
                        const int row_offset, const int col_offset,
                        const int row_size, const int col_size) const {
    int dim_x = N * T * n;
    int dim_u = N * T * m;
    auto &drdx = mtx;
    // Initialize index
    ...
    for (int i = 0; i < N; ++i) {
      ...
      Matrix dLL_dxi_dx = dLLi_dxi_dx(x, u, h_plus_mask, lamda, mu, i);
      // drdx.block(index, 0, T * n, dim_x) = dLL_dxi_dx;
      sp_assign(dLL_dxi_dx, drdx, index + row_offset, 0 + col_offset, T * n,
                dim_x);
      index += T * n + T * m;
      ...
      for (int k = 1; k < T; ++k) {
        Matrix dFdx = dF_dx(x, u, i, k);
        // drdx.block(index + n * k, 0, n, dim_x) = dFdx;
        sp_assign(dFdx, drdx, index + n * k + row_offset, 0 + col_offset, n,
                  dim_x);
      }
      index += n * T;
      ...
      for (int k = 1; k <= T; ++k) {
        ...
        // TODO we could set the original matrix directly to avoid copying
        // drdx.block(index, 0, dhdx.rows(), dim_x) = dhdx;
        sp_assign(dhdx, drdx, index + row_offset, 0 + col_offset, dhdx.rows(),
                  dim_x);
        index += dhdx.rows();
      }
    }
  ...
  template <typename Derived>
  void dr_du(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
             const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
             const std::vector<Matrix> &h_plus_mask,
             const MatrixBase<Derived> &mtx) const {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      drdu.template block<m, m>(index + k * m, k * N * m + i * m) =
          dLL_duik_duik;
      for (int k = 1; k < T; ++k) {
        // dLL_duik_duik
      ...
      drdu.template block<n, m>(index + 0 * n, 0 * N * m + i * m) =
          df_du(x0.row(i).transpose(), u[0].row(i).transpose(), i);
      for (int k = 1; k < T; ++k) {
        drdu.template block<n, m>(index + k * n, k * N * m + i * m) =
            df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i);
      }
  ...
  Matrix dr_du(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
               const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
               const std::vector<Matrix> &h_plus_mask) const {
    // Calculate dimensions
    int dim_u = T * N * m;
  ...
  void dr_du_fill_block(const std::vector<Matrix> &x,
                        const std::vector<Matrix> &u,
                        const std::vector<Matrix> &lamda,
                        const std::vector<Matrix> &mu,
                        const std::vector<Matrix> &h_plus_mask, SpMatrix &mtx,
    ...
    for (int i = 0; i < N; ++i) {
      ...
      // drdu.template block<m,m>(index + k * m, k * N * m + i * m) =
      // dLL_duik_duik;
      sp_assign(dLL_duik_duik, drdu, index + k * m + row_offset,
                k * N * m + i * m + col_offset, m, m);
      for (int k = 1; k < T; ++k) {
        // dLL_duik_duik
        Matrix dLL_duik_duik = dJi_dudu(x[k - 1], u[k].row(i).transpose(), i);
        // drdu.template block<m,m>(index + k * m, k * N * m + i * m) =
      ...
      // drdu.template block<n,m>(index + 0 * n, 0 * N * m + i * m) =
      // df_du(x0.row(i).transpose(), u[0].row(i).transpose(),i);
      sp_assign(df_du(x0.row(i).transpose(), u[0].row(i).transpose(), i), drdu,
                index + 0 * n + row_offset, 0 * N * m + i * m + col_offset, n,
                m);
      for (int k = 1; k < T; ++k) {
        // drdu.template block<n,m>(index + k * n, k * N * m + i * m) =
        // df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(),i);
        sp_assign(
            df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i),
            drdu, index + k * n + row_offset, k * N * m + i * m + col_offset, n,
            m);
      }
      // skip count for h_plus_mask[all k, i, all j]
      int skip_count = 0;
  ...
  template <typename Derived>
  void dr_dlamda(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                 const std::vector<Matrix> &lamda,
                 const std::vector<Matrix> &mu,
                 const std::vector<Matrix> &h_plus_mask,
                 const MatrixBase<Derived> &mtx) const {
    ...
    for (int i = 0; i < N; ++i) {
      for (int k = 1; k < T; ++k) {
        // dLLi_dxki_dlamda_ki
        drdlamda.template block<n, n>(index + (k - 1) * n, k * N * n + i * n) =
            df_dx(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i)
                .transpose();
        // dLLi_dxki_dlamda_k-1,i
        drdlamda.template block<n, n>(index + (k - 1) * n,
                                      (k - 1) * N * n + i * n) =
            -Matrix::Identity(n, n);
      }
      ...
      drdlamda.template block<m, n>(index + 0 * m, 0 * N * n + i * n) =
          df_du(x0.row(i).transpose(), u[0].row(i).transpose(), i).transpose();
      for (int k = 1; k < T; ++k) {
        drdlamda.template block<m, n>(index + k * m, k * N * n + i * n) =
            df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i)
                .transpose();
      }
  ...
  Matrix dr_dlamda(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                   const std::vector<Matrix> &lamda,
                   const std::vector<Matrix> &mu,
                   const std::vector<Matrix> &h_plus_mask) const {
    // Calculate dimensions
  ...
  void dr_dlamda_fill_block(const std::vector<Matrix> &x,
                            const std::vector<Matrix> &u,
                            const std::vector<Matrix> &lamda,
                            const std::vector<Matrix> &mu,
                            const std::vector<Matrix> &h_plus_mask,
                            SpMatrix &mtx, const int row_offset,
                            const int col_offset, const int row_size,
                            const int col_size) const {
    auto &drdlamda = mtx;
    int index = 0;
    for (int i = 0; i < N; ++i) {
      for (int k = 1; k < T; ++k) {
        // dLLi_dxki_dlamda_ki
        // drdlamda.template block<n,n>(index + (k - 1) * n, k * N * n + i * n)
        // = df_dx(x[k - 1].row(i).transpose(),
        // u[k].row(i).transpose(),i).transpose();
        sp_assign(df_dx(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i)
                      .transpose(),
                  drdlamda, index + (k - 1) * n + row_offset,
                  k * N * n + i * n + col_offset, n, n);
        // dLLi_dxki_dlamda_k-1,i
        // drdlamda.template block<n,n>(index + (k - 1) * n, (k - 1) * N * n + i
        // * n) = -Matrix::Identity(n, n);
        sp_assign(-Matrix::Identity(n, n), drdlamda,
                  index + (k - 1) * n + row_offset,
                  (k - 1) * N * n + i * n + col_offset, n, n);
      }
      ...
      sp_assign(
          df_du(x0.row(i).transpose(), u[0].row(i).transpose(), i).transpose(),
          drdlamda, index + 0 * m + row_offset, 0 * N * n + i * n + col_offset,
          m, n);
      for (int k = 1; k < T; ++k) {
        // drdlamda.template block<m,n>(index + k * m, k * N * n + i * n) =
        // df_du(x[k-1].row(i).transpose(),
        // u[k].row(i).transpose(),i).transpose();
        sp_assign(df_du(x[k - 1].row(i).transpose(), u[k].row(i).transpose(), i)
                      .transpose(),
                  drdlamda, index + k * m + row_offset,
                  k * N * n + i * n + col_offset, m, n);
      }
  ...
  template <typename Derived>
  void dr_dmu(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
              const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
              const std::vector<Matrix> &h_plus_mask,
              const MatrixBase<Derived> &mtx) const {
  ...
  Matrix dr_dmu(const std::vector<Matrix> &x, const std::vector<Matrix> &u,
                const std::vector<Matrix> &lamda, const std::vector<Matrix> &mu,
                const std::vector<Matrix> &h_plus_mask) const {
    // Calculate dimensions
    const int dim_mu = T * N * N;
  ...
  void dr_dmu_fill_block(const std::vector<Matrix> &x,
                         const std::vector<Matrix> &u,
                         const std::vector<Matrix> &lamda,
                         const std::vector<Matrix> &mu,
                         const std::vector<Matrix> &h_plus_mask, SpMatrix &mtx,
    ...
    for (int i = 0; i < N; ++i) {
      ...
      Matrix dLL_dxi_dmu = dLLi_dxi_dmu(x, u, h_plus_mask, lamda, mu, i);
      // drdmu.block(index, 0, T*n, dim_mu) = dLL_dxi_dmu;
      sp_assign(dLL_dxi_dmu, drdmu, index + row_offset, 0 + col_offset, T * n,
                dim_mu);
      // skip count for h_plus_mask[all k, i, all j]
      int skip_count = 0;
      for (int k = 1; k < T + 1; ++k) {
  ...
  std::vector<Matrix> getHplusMask(const std::vector<Matrix> &x) const {
    ...
    for (int k = 1; k < T + 1; ++k) {
      for (int i = 0; i < N; ++i) {
        for (int j = i + 1; j < N; ++j) {
          h_plus_mask[k - 1](i, j) = h_plus_mask[k - 1](j, i) =
              h(x[k - 1].row(i).transpose(), x[k - 1].row(j).transpose()) >= 0;
        }
      }
    }
    return h_plus_mask;
  ...
  Scalar getCollisionResidual(const std::vector<Matrix> &x) const {
    Scalar h_res = 0;
    for (int k = 1; k < T + 1; ++k) {
      for (int i = 0; i < N; ++i) {
        for (int j = i + 1; j < N; ++j) {
  ...
  virtual Matrix f(const Matrix x, const Matrix u, const int i) const {
    throw std::runtime_error("abstract function f() shouldn't be called");
    return x;
  }
  virtual Matrix df_dx(const Matrix x, const Matrix u, const int i) const {
    throw std::runtime_error("abstract function df_dx() shouldn't be called");
    return x;
  }
  virtual Matrix df_du(const Matrix x, const Matrix u, const int i) const {
  ...
  virtual Scalar h(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error("abstract function h() shouldn't be called");
    return 0.0;
  }
  virtual Matrix dh_dxi(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error("abstract function dh_dxi() shouldn't be called");
    return x_i;
  }
  virtual Matrix dh_dxj(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error("abstract function dh_dxj() shouldn't be called");
    return x_i;
  }
  virtual Matrix dh_dxi_dxi(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error(
        "abstract function dh_dxi_dxi() shouldn't be called");
    return x_i;
  }
  virtual Matrix dh_dxj_dxi(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error(
        "abstract function dh_dxj_dxi() shouldn't be called");
    return x_i;
  }
  virtual Matrix dh_dxi_dxj(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error(
        "abstract function dh_dxi_dxj() shouldn't be called");
    return x_i;
  }
  virtual Matrix dh_dxj_dxj(const Matrix x_i, const Matrix x_j) const {
    throw std::runtime_error(
        "abstract function dh_dxj_dxj() shouldn't be called");
    return x_i;
  }

  // Objective function (J)
  virtual Matrix J(const Matrix x_k, const Matrix u_k_i, int i) const {
    throw std::runtime_error("abstract function J() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dxi(const Matrix x_k, const Matrix u, int i) const {
    throw std::runtime_error("abstract function dJi_dxi() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dxj(const Matrix x_k, const Matrix u, int i, int j) const {
    throw std::runtime_error("abstract function dJi_dxj() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_du(const Matrix x_k, const Matrix u, int i) const {
    throw std::runtime_error("abstract function dJi_du() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dxi_dxi(const Matrix x_k, const Matrix u, int i) const {
    throw std::runtime_error(
        "abstract function dJi_dxi_dxi() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dxi_dxj(const Matrix x_k, const Matrix u, int i,
                             int j) const {
    throw std::runtime_error(
        "abstract function dJi_dxi_dxj() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dxj_dxj(const Matrix x_k, const Matrix u, int i,
                             int j) const {
    throw std::runtime_error(
        "abstract function dJi_dxj_dxj() shouldn't be called");
    return x_k;
  }
  virtual Matrix dJi_dudu(const Matrix x_k, const Matrix u, int i) const {
    throw std::runtime_error(
        "abstract function dJi_dudu() shouldn't be called");
    return x_k;
  }
};
